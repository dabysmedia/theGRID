...
describe("stepsDayKey", () => {
  it("uses calendar date at and after 07:00 local", () => {
    expect(stepsDayKey(atLocal("2026-07-06", 7, 0), TZ)).toBe("2026-07-06")
  ...
  it("attributes pre-07:00 local time to the previous calendar day", () => {
    // Tuesday 2am → Monday's steps day
    expect(stepsDayKey(atLocal("2026-07-07", 2, 0), TZ)).toBe("2026-07-06")
    expect(stepsDayKey(atLocal("2026-07-07", 6, 59), TZ)).toBe("2026-07-06")
  })

  it("switches to the new steps day exactly at 07:00", () => {
    expect(stepsDayKey(atLocal("2026-07-07", 6, 59), TZ)).toBe("2026-07-06")
    expect(stepsDayKey(atLocal("2026-07-07", 7, 0), TZ)).toBe("2026-07-07")
  })
})

describe("getStepsDayRange", () => {
  it("returns [07:00, next 07:00) in the given timezone", () => {
...
describe("stepsRefDayKey", () => {
  it("remaps calendar today before 7am to the previous steps day", () => {
...
describe("bucketStepsByStepsDay", () => {
  it("puts late-night and early-morning hours on the previous steps day", () => {
    ...
    const tue07 = atLocal("2026-07-07", 7, 0)
