...
export const PAGE_FOOTER_DOCK_CLEARANCE =
  "min-h-[var(--bottom-dock-clearance)]" as const
...
interface PageFooterProps {
  ...
  showRail?: boolean
  /**
   * Hub overview: no END rail / top margin — clearance alone reserves dock space
   * so the hub can fill the viewport without default page scroll.
   */
  dockOnly?: boolean
}
...
export function PageFooter({
  ...
  return (
    <footer
      role="contentinfo"
      className={cn(dockOnly ? "mt-0 shrink-0" : "mt-10 shrink-0", className)}
      aria-label="Page end"
    >
      {showRail && !dockOnly && (
